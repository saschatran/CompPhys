...
! @note: The file md.f90 was modified at the lines with "ADD THIS PART TO THE PROGRAM".
!
!        Additionally, in line 112 the positions of the atoms are saved in a folder 
!        and therefore the path was modified to 'pos/'//filename. Make sure to have a 'pos' folder.
!        The files are .xyz files instead of .ascii files
!
!        After running the program, the folder pos was changed to hotPos and coldPos respectively.
!        The same goes for the file MDiter which will be saved to MDITER_hot resp. MDITER_cold.
!        The content of the file bazant_lib.f90 was added (line 131+).
...
! alat is the size of the simulation cell
        ...
        open(unit=2,file='MDiter',status='unknown')

        open(unit=1,file='cold.dat',status='old')       !open(unit=1,file='hot.dat',status='old')
! read alat,nat
        read(1,*) alat(1),alat(2),alat(3)
        read(1,*) nat
        write(*,'(a,x,i7,3(x,e12.5))') 'nat,alat',nat,alat

        write(*, *) "Make sure you have a folder called 'pos'"

! allocate arrays
        allocate(rxyz(3,nat),vxyz(3,nat),fxyz(3,nat),gxyz(3,nat))
...
! ADD THIS PART TO THE PROGRAM
        ...
        rkin = 0
        do i=1,nat
                norm = vxyz(1,i)**2 + vxyz(2,i)**2 + vxyz(3,i)**2
                v = v + norm
                rkin = rkin + norm * rmass * 0.5
        enddo

        temp = rmass / ((3 * nat - 3) * rkboltz) * v
...
!  generate filename and open files
        ...
        filename = 'pos'//fn//'.xyz'
        write(6,*) filename
        open(unit=1,file='pos/'//filename,status='unknown')

        write(1,*) nat                                          ! '        ' changed to nat
        write(1,'(3(x,e14.7))') alat(1),0.d0,alat(2)
        write(1,'(3(x,e14.7))') 0.d0,0.d0,alat(3)
        do iat=1,nat
        write(1,'(a,3(x,e14.7))') 'Si', (rxyz(j,iat),j=1,3)
        enddo
        close(1)
        endif
